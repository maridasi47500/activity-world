...
(defn render-collection-params-athletes
  ...
  (let [body (if (seq coll)
    ...
    (println "body:" body)
    body))
(defn render-collection-params-results
  "Render a collection of news into a template with name of params in view."
  [title coll template]
  (println "coll:" coll)
  (println "title:" title)
  ...
  (let [body (if (seq coll)
               (str/join "" (map #(replace-several (slurp (io/resource template))
                 "$mytime" (str (:mytime %))
                 "$live_schedule_id" (str (:live_schedule_id %))
                 "$athleteimage" (str (:athleteimage %))
                 "$athlete_name" (str (:athletename %))
...
[:div (hf/form-to {:id "form-edit-album"} [:post "/action_update_album"]
    [:div
         (anti-forgery-field)
         (hf/hidden-field "album[id]" id)    
      ]


    [:div
         (hf/label "album[title]" "title")    
         (hf/text-field "album[title]" title)    
      ]
    [:div
         (hf/label "album[subtitle]" "subtitle")    
         (hf/text-field "album[subtitle]" subtitle)    
      ]
...
[:div (hf/form-to {:id "form-edit-news"} [:post "/action_update_news"]
    [:div
         (anti-forgery-field)
         (hf/hidden-field "news[id]" id)    
      ]
    ...
    [:div
         ...
         (hf/text-area "news[content]" content)    
      ]
         (hf/submit-button "Submit"))]) #"(<html>|<\/html>)" "")
                 )
(defn form-news-page
...
(defn results [params]
  (println "request results : ", (:params params))
  (println "request results : ", (get (:params params) "result_id"))
  (if (get (:params params) "live_schedule_id")
  (let [reqres (:params params)
        result_id (get reqres "result_id")
        live_schedule_id (get reqres "live_schedule_id")
        live_schedule (db/get-live-schedule-by-id live_schedule_id)
        event (db/get-event-by-id (:event_id live_schedule))
        activity (db/get-activity-by-id (:activity_id live_schedule))
        results (db/get-results-by-live-schedule-id live_schedule_id)
        template (slurp (io/resource "results.html"))
        filled-template (replace-template template
                         {
                          "$eventtitle" (:title event)
                          "$activityname" (:name activity)
                          "$date_debut" (:date_debut event)
                          "$date_fin" (:date_fin event)
                          "$mytitle" (:title live_schedule)
                          "$mytime" (:mytime live_schedule)
                          "$mydate" (:mydate live_schedule)
                          "$allevents" (render-collection-params-events "results" (db/get-events) "_optionevent.html")
                          "$allschedules" (render-collection-params-live-schedule "activities" (db/get-live-schedules) "_optionliveschedule.html")
                          "$allresults" (render-collection-params-results "results" results "_listresult.html")
    ...
    (response/content-type
      (response/response final-page)
      "text/html"))
  (let [template (slurp (io/resource "myresults.html"))
        filled-template (replace-template template
                         {
                          "$allevents" (render-collection-params-events "results" (db/get-events) "_optionevent.html")
                          "$allschedules" (render-collection-params-live-schedule "activities" (db/get-live-schedules) "_optionliveschedule.html")})
...
(defn action-update-album [request]
  (let [params   (:multipart-params request)
        id       (get params "album[id]")
        title    (get params "album[title]")
        subtitle (get params "album[subtitle]")]

    (if (and id title subtitle)
      (do
        (println "Mise à jour de l'album : " id title subtitle)
        (db/update-album! {:id id
                           :title title
                           :subtitle subtitle})

        (response/content-type
         (response/response
          (replace-several (render-json "indexalbumid.json") "$album_id" (str id)))
         "application/json"))

      ;; Champs manquants
      (do
        (println "Champs manquants — mise à jour impossible.")
        (response/content-type
         (response/response (render-json "myform.json"))
         "application/json")))))


(defn action-update-news [request]
  (println "request" (str request))
  (println "Headers:" (:headers request))
  (println "Multipart params:" (:multipart-params request))
  (println "Form params:" (:form-params request))
      ...
      (do
        ...
        (if (and (map? photo)
          ...
          (let [filename      (:filename photo)
            ...
            (println "New photo uploaded — saving to disk.")
            (clojure.java.io/make-parents target-path)
            (clojure.java.io/copy tempfile (clojure.java.io/file target-path))
          ...
          ;; No new photo — update only title and content
          (do
        ...
        (response/content-type
         (response/response
          (replace-several (render-json "indexnewsid.json") "$news_id" (str id)))
         "application/json"))

      ;; Missing title or content
      (do
        (println "Missing title or content — cannot update.")
        (response/content-type
         (response/response (render-json "myform.json"))
         "application/json")))))
...
(defn action-update-video [request]
  (println "request" (str request))
  (println "Headers:" (:headers request))
  (println "Multipart params:" (:multipart-params request))
  (println "Form params:" (:form-params request))
      ...
      (do
        ...
        (if (and (map? myvideo)
          ...
          (let [filename       (:filename myvideo)
            ...
            (println "New video uploaded — saving to disk.")
            (clojure.java.io/make-parents target-path)
            (clojure.java.io/copy tempfile (clojure.java.io/file target-path))

            ;; Update all fields including video
            (db/update-video! {:id id
                               :title title
                               :content content
                               :myvideo safe-filename}))

          ;; No new video — update only title and content
          (do
        ...
        (response/content-type
         (response/response
          (replace-several (render-json "indexvideoid.json") "$video_id" (str id)))
         "application/json"))
